...
export function transformLink(src: FullSlug, target: string, opts: TransformOptions): RelativeURL {
  ...
  if (opts.strategy === "relative") {
    ...
    if (opts.strategy === "shortest") {
      ...
      // Obsidian-style partial path matching: if target contains '/',
      // match slugs that end with the target path (e.g. "DT1/AMI295"
      // matches "07-Kursplaner/DT1/AMI295")
      if (targetCanonical.includes("/")) {
        const suffixMatch = opts.allSlugs.filter((slug) => {
          return slug === targetCanonical || slug.endsWith("/" + targetCanonical)
        })
        if (suffixMatch.length === 1) {
          const targetSlug = suffixMatch[0]
          return (resolveRelative(src, targetSlug) + targetAnchor) as RelativeURL
        }
      }
    }
